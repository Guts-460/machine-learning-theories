...
# 13. 大批量结构的PCA
在分析蛋白质系综的主成分分析（PCA）时，无需对所有结构同时计算本征矢。<br>

一种计算高效的替代方法如下：

**抽样与预处理**：从全集结构中随机抽取一个子集，并以天然态（或其他选定参考态）为基准进行对齐。具体包含两步：首先基于Cα原子进行结构对齐，然后通过平移和旋转将坐标居中（即与参考态对齐）。

**降维基底计算**：基于该小样本子集计算协方差矩阵的本征矢。保留前两个主成分（记作 $𝑤_1$, $w_2$）作为降维投影基底。

**全集投影**：对于任一后续新结构，依次经过（1）与同一参考态进行Cα对齐和坐标居中，（2）向 $𝑤_1$, $w_2$方向投影，即可实现降维
